...
//    }
    ...
    func goBack() {
        ...
        performSegueWithIdentifier("backToNetworkFromMeetings", sender: self)
    }
...
//        self.view.addGestureRecognizer(self.revealViewController().panGestureRecognizer())
        ...
        } else if fromVC == "network" {
            
            // Set up the segmented controll to only have two segments
            segment.removeSegmentAtIndex(2, animated: false)
            segment.setTitle("Kommende", forSegmentAtIndex: 0)
            segment.setTitle("Afholdte", forSegmentAtIndex: 1)
            
            
            let specificUrl = "https://brilliant-torch-4963.firebaseio.com/networkgroups/" + self.fromNetworkId + "/meetings"
            //print(specificUrl)
